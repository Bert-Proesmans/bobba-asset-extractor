...
#[derive(Debug)]
enum BobbaError {
    FurniDownload(StatusCode),
    // SwfParseError(SwfParseError),
    Other(String),
}

impl fmt::Display for BobbaError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            BobbaError::FurniDownload(ref status_code) => write!(
                f,
                "Furniture data download error. Expected status code 2XX, got {}. Try to manually download the file through a browser!",
                status_code
            ),
           //  BobbaError::SwfParseError(ref parse_error) => write!(f, "SWF Parsing failure: {:?}", parse_error),
            BobbaError::Other(ref string) => write!(f, "Constraint error: {}", string),
        }
    }
}

impl Error for BobbaError {}
